...
class feedback_handler(gr.basic_block):
    ...
    def handle_msg(self, msg):
        # Call the callback to process the feedback message.
        self.callback(msg)
        # Forward the message downstream (if needed).
        self.message_port_sub(pmt.intern("out"), msg)
...
class wifi_tx(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        self.freq = freq = self.channel_mapping[self.channel_flag]
        ##################################################
        # GUI Botton group
        ##################################################
        # Create the tx gain list and button group
        ...
        # Create the offset list and button group
        ...
        # Create the message send invertal (ms) and slider
        ...
        # Create the frequency list and button group
        ...
        # Create the encoding list and button group
        self._encoding_options = [0, 1, 2, 3, 4, 5]
        self._encoding_labels = ['BPSK 1/2', 'QPSK 1/2', '16QAM 1/2', '16QAM 3/4', '64QAM 2/3', '64QAM 3/4']
        ...
        self.zeromq_sub_msg_source_0 = zeromq.sub_msg_source('tcp://192.168.0.76:54321', 100, False)
        ...
        self.blocks_message_strobe_0_0 = blocks.message_strobe(pmt.intern("Hellowworld"), interval)
        ...
        # Instantiate the feedback handler block with our callback.
        self.feedback_handler = feedback_handler(self.handle_feedback)
        self.msg_connect((self.zeromq_sub_msg_source_0, 'out'), (self.feedback_handler, 'in'))
        self.msg_connect((self.feedback_handler, 'out'), (self.ieee802_11_mac_0, 'app in'))
        ...
        # For debugging
        self.mod_print_timer = QtCore.QTimer(self)
        self.mod_print_timer.setInterval(5000)  # 5000 milliseconds = 5 seconds
        self.mod_print_timer.timeout.connect(self.print_channel)
        self.mod_print_timer.start()

    def print_channel(self):
        self.channel_flag = self.get_channel_from_mapping(self.channel_mapping, int(self.get_freq()))
        print("Current channel in sue:", self.channel_flag)
    ...
    def get_channel_from_mapping(self,map_dict, freq):
        return [key for key, value in map_dict.items() if value == freq]
    ...
    # Check that the message is a PDU (a PMT pair) and extract the data.
        print("Received feedback message:", msg)
        if pmt.is_pair(msg):
            meta = pmt.car(msg)
            data = pmt.cdr(msg)
            if pmt.is_number(data):
                channel_flag = pmt.to_long(data)
                print("Received feedback channel flag:", channel_flag)
                # Update the modulation encoding accordingly.
                self.channel_flag = channel_flag
                freq = self.channel_mapping[self.channel_flag]
                self.set_freq(freq)
