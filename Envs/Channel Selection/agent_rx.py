...
from model_storage import DQN_CNN, DQN_MLP
...
class Agent():
    ...
    def __init__(self, state_size, action_size, dqn_type='DQN', replay_memory_size=1e5, batch_size=64, gamma=0.99,
        ...
        self.state_dim = 1 # default to 1 for CNN
        ...
        if type(state_size) == int:
            self.network = DQN_MLP(state_size, action_size, seed).to(device)
            self.target_network = DQN_MLP(state_size, action_size, seed).to(device)
        else:
            self.network = DQN_CNN(self.state_dim, action_size, seed).to(device)
            self.target_network = DQN_CNN(self.state_dim, action_size, seed).to(device)
